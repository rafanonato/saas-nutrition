...
import { Mic, MicOff, ChevronRight, FileText, Activity, PieChart, Send, Sparkles, Building2, Shield, Users } from 'lucide-react';
import { ActiveTab, PatientSummary, ClinicTenant, NutritionistUser } from '../types';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const isClinicManagement = activeTab === 'gestao-clinica';
  ...
  return (
    ...
    >
      {/* 1. Informações do Paciente Ativo OU Identidade da Clínica */}
      {isClinicManagement ? (
        <div className="flex items-center gap-3.5">
          <div 
            id="clinic-avatar-badge"
            className="w-11 h-11 rounded-2xl bg-indigo-100 text-indigo-800 font-bold flex items-center justify-center text-sm shadow-xs border border-indigo-200"
          >
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-semibold text-slate-900 text-base tracking-tight">
                {clinic?.tradeName || clinic?.name || 'Gestão da Clínica'}
              </h1>
            ...
            <div className="flex items-center gap-3 text-xs text-slate-500 mt-0.5">
              <span className="font-medium text-slate-700">
                Camada 1: Governança & Cadastro de Nutricionistas
              </span>
              <span className="text-slate-400 hidden sm:inline">
                • Sigilo ético de prontuários preservado
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        {isClinicManagement && (
          <button
            ...
            className="px-3.5 py-1.5 rounded-full text-xs font-semibold bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 transition cursor-pointer flex items-center gap-1.5 shadow-2xs"
            title="Ir para a carteira de pacientes do consultório"
          >
            <Users className="w-3.5 h-3.5" />
            <span>Ver Pacientes da Nutri</span>
          </button>
        )}
        ...
        <button
          ...
          onClick={() => {
            ...
            else if (activeTab === 'gestao-clinica') onSelectTab('pacientes');
        ...
        >
          <span>
            ...
            {activeTab === 'pacientes' && 'Ir para Atendimento'}
            {activeTab === 'gestao-clinica' && 'Ir para Pacientes'}
